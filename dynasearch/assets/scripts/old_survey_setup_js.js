...
var add_training_page = function()
{
	var newitem = new Element('div');
	newitem.setAttribute('class', 'page_bar');
...
{
	var experiment_name = $('survey_name').innerHTML;
	   ...
	   // Seperated by Jon
		if(type == 'Survey Page'){		
	      ...
	      req = new Request({
		         ...
		         async: false, method: 'post', 
		         data: {
			          'name': c[2].getFirst('.questionairre_name').innerHTML,
			          'file_path': filepath
			      }, 
		          onComplete: function(r) {} 
	      }).send();
		...
		else if(type == 'Information Page')
		{
		   ...
		   req = new Request({
		         ...
		         async: false, method: 'post', 
		         data: {
			          'fileName': c[1].getFirst('.page_source').innerHTML,
			          'expName': short_name,
			      }, 
		          onComplete: function(r) {} 
	      }).send();
		...
		else
		{
			var page_title = c[0].getFirst('.page_title').innerHTML;
			var page_src   = c[1].getFirst('.page_source').innerHTML;
			//var adv_num    = c[2].getFirst('.advisory_number').innerHTML
