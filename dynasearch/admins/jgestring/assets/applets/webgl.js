...
var CAMERA_POSITION = [200.0,300.0,500.0],
...
//              Filename      ID        Position            Scale     orientation?
var MODELS = [ "duck.dae" , "earth1", [0.0, 0.0, 0.0],  [1, 1, 1],
               "globe.dae" , "earth2", [0.0, 0.0, 0.0],  [1, 1, 1]   ];
...
function myWebGlInit() {
   c3dl.addMainCallBack(canvasMain, "webgl_webgl");
   preloadModels();
   c3dl.init();
}
...
function canvasMain( canvasName ) {
   ...
   if ( renderer.isReady() ) {
      ...
      scn.setPickingPrecision(c3dl.PICK_PRECISION_BOUNDING_VOLUME);
...
if (objectsPicked.length != 0) { alert(objectsPicked.length); }
      if (buttonUsed == 1) {
